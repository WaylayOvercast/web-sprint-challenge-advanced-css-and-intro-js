...
let getArtistByIndex =(array, index)=> {
  let array1 = array[index]
  return `the artist at index ${index} is ${array1.name}`
}
...
function get20s(arr) {
  ...
  let birth = parseInt(item.years.substring(0, 4))
  let death = parseInt(item.years.substring(item.length -4, item.length))
...
function addArtist(arr) {
 ...
 ar.push({ 
  ...
  "bio": "i live in oklahoman, and i like to code. and now this is just randome stuff im adding to the bio to fill it in a little more, yes."
})
return ar
}
// i dont know whats wrong here it looks fine to me//
...
function lotsOfArt(arr) {
  let a = arr.filter(item => item.paintings >= 100)
  return a.map(art => art.name)
}
